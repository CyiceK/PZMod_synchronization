...
from utils.save_map_window_utils import MapEntry, SaveModMapEntry
...
class MapDataMixin:
    def _collect_maps(self, config_override: Optional[Path] = None) -> List[MapEntry]:
        ...
        # When save version != game directory version (e.g. B41 save + B42 game),
        # replace vanilla maps with bundled entries, keep only mod maps
        bundled_entries: List[MapEntry] = []
        game_dir_prefixes: List[str] = []
        is_bundled_fallback = self._is_bundled_tile_fallback()
        if is_bundled_fallback:
        ...
        log_service.debug(
            f"[MapScan] _collect_maps: map_order={map_order} "
            f"map_filter={map_filter} use_filter={use_filter} "
            f"bundled={len(bundled_entries)} candidates_1st={len(candidates)}"
        )
        if not candidates and map_filter:
            candidates = self._find_maps(set())
            log_service.debug(
                f"[MapScan] _collect_maps: fallback no-filter "
                f"candidates_2nd={len(candidates)}"
            )

        # Filter out game-directory vanilla maps when using bundled fallback
        if bundled_entries:
            def _is_game_dir_map(entry: MapEntry) -> bool:
                """Check if a map entry comes from the game directory."""
                try:
            ...
            # Also filter workshop/mod maps that are NOT in the save's active
            # mod list.  When bundled fallback is active, stray workshop maps
            # (possibly for a different game version) should not leak in.
            bundled_names = {e.name.lower() for e in bundled_entries}
            active_mod_ids = self._active_mods or set()
            active_ws_ids = self._active_workshops or set()

            def _should_keep(entry: MapEntry) -> bool:
                # Always reject game-directory maps
                if _is_game_dir_map(entry):
                    return False
                # If the entry has the same name as a bundled entry, skip it
                # (the bundled version takes priority)
                if entry.name.lower() in bundled_names:
                    return False
                # If the entry is from a mod, only keep it if the mod is active
                mod_id = getattr(entry, "mod_id", None) or ""
                ws_id = self._get_workshop_id(entry.path)
                if mod_id or ws_id:
                    mod_key = mod_id.lower() if mod_id else ""
                    if active_mod_ids and mod_key and mod_key in active_mod_ids:
                        return True
                    if active_ws_ids and ws_id and ws_id in active_ws_ids:
                        return True
                    # No active mods/workshops configured — only keep if the
                    # map name is in the server map list (explicit allowlist).
                    if map_filter and self._normalize_map_name(entry.name) in map_filter:
                        return True
                    log_service.debug(
                        f"[MapScan] _collect_maps: dropping stray mod map "
                        f"name={entry.name!r} mod_id={mod_id!r} path={entry.path}"
                    )
                    return False
                # Non-mod entry with different name — keep only if in map list
                if map_filter:
                    return self._normalize_map_name(entry.name) in map_filter
                return True

            mod_only = [e for e in candidates if _should_keep(e)]
            log_service.debug(
                f"[MapScan] _collect_maps: after filter "
                f"before={len(candidates)} mod_only={len(mod_only)} "
                f"names={[e.name for e in mod_only]}"
            )
        ...
        log_service.debug(
            f"[MapScan] _collect_maps: final maps={len(unique)} "
            f"names={[e.name for e in unique]} "
            f"worldmaps={[str(e.worldmap) if e.worldmap else None for e in unique]} "
            f"base_map={self._base_map.name if self._base_map else None}"
        )
        return unique
    ...
    def _is_bundled_tile_fallback(self) -> bool:
        """Check if we should use bundled tiles due to version mismatch."""
        try:
            ...
            log_service.debug(
                f"[MapScan] bundled_tile_fallback: save_build={save_build} "
            ...
            try:
                from services.log_service import log_service
                log_service.debug(
                    f"[MapScan] bundled_tile_fallback: exception={exc!r}"
                )
            except Exception:
                pass
            return False
    ...
    def _build_bundled_map_entries(self) -> List[MapEntry]:
        """Build MapEntry list from bundled tile manifest for version fallback."""
        try:
            from services.bundled_tile_service import BundledTileService
            from services.log_service import log_service
            svc = BundledTileService.instance()
            ...
            for m in maps_defs:
                ...
                # Resolve bundled worldmap.xml for correct-version features
                worldmap_path = None
                wm_name = m.get("worldmap")
                if wm_name and tiles_root:
                    wm = tiles_root / wm_name
                    if wm.exists():
                        worldmap_path = wm
            ...
            log_service.debug(
                f"[MapScan] bundled_map_entries: build={build_key} "
                f"count={len(entries)} names={[e.name for e in entries]}"
            )
            return entries
        except Exception:
            return []
        ...
        try:
            ...
            if build_key is not None:
                ...
                if tiles_root is not None:
                    ...
                    log_service.debug(
                        f"[MapPreview] tile_root: bundled_fallback "
                        f"build={build_key} origin={self._tile_origin} "
    ...
    def _load_features(self, maps: List[MapEntry]) -> List[Tuple[str, List[Tuple[float, float]]]]:
        ...
        for entry in maps:
            if entry.worldmap is None:
                log_service.debug(
                    f"[MapScan] _load_features: skip {entry.name!r} (no worldmap)"
                )
                continue
            ...
            after = len(features)
            log_service.debug(
                f"[MapScan] _load_features: {entry.name!r} "
                f"worldmap={entry.worldmap} offset={offset} "
                f"added={after - before} total={after}"
            )
        return features
