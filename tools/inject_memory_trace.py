...
Documentation translated to English.
python tools/inject_memory_trace.py <>
...
class MemoryCheckpoint:
    """Documentation translated to English."""_init__(self, name: str):
        self.name = name
...
class DetailedMemoryTracer:
    """Documentation translated to English."""it__(self):
        self.checkpoints: List[MemoryCheckpoint] = []
        self.start_time = datetime.now()

    def start(self):
        """Documentation translated to English.""" tracemalloc.start(25)
        print(f"[Tracer] 内存追踪已启动 @ {self.start_time}")

    def checkpoint(self, name: str) -> MemoryCheckpoint:
        """Documentation translated to English."""p = MemoryCheckpoint(name)
    ...
    def generate_report(self) -> str:
        """Documentation translated to English."""es = []
        lines.append("=" * 100)
    ...
    def save_report(self, filename: str = None) -> Path:
        """Documentation translated to English.""" if filename is None:
...
def trace_map_scan(save_path: Path):
    """Documentation translated to English."""dMemoryTracer()
    tracer.start()
