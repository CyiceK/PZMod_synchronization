...
class PerformanceProfiler:
    ...
    def compare_profiles(
        ...
        Args:
            test_func: 测试函数
            profiles: 要测试的性能档案列表（默认全部）
            iterations: 每个档案的测试次数
    ...
    def generate_report(
        ...
        report = PerformanceReport(
            ...
            comparisons=[],  # 需要通过 compare_profiles 填充
...
class _ProfileContext:
    ...
    def __init__(self, profiler: PerformanceProfiler, name: Optional[str] = None):
...
def measure(name: str, iterations: int = 1) -> Callable:
    """便捷函数: 多次测量装饰器"""
    return get_profiler().measure(name, iterations)
