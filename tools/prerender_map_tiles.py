#!/usr/bin/env python3
"""(Map Tile Pre-Renderer)

WebP
resources/map_tiles/

Documentation translated to English.
B41: media/maps/{MapName}/pyramid.zip → 0/tile{X}x{Y}.png ()
B42: media/maps/{MapName}/maps/biomemap_{X}_{Y}.png ()
...
def _read_game_path_from_config() -> Optional[str]:
    """game_path PyQt6"""OT / "user_data" / "config.json"
...
class TileSource:
    ...
    def __init__(
        ...
        *,
        ...
        self.x = x
        self.y = y
        self.path = path           # Comment translated to English.
        self.zip_path = zip_path   # zip
        self.zip_entry = zip_entry # zip

    def load_bytes(self) -> bytes:
        """PNG"""lf.path is not None:
...
def discover_tiles_cell(game_dir: Path) -> list[TileSource]:
    """/: media/textures/mapTiles/cell_{X}_{Y}.png"""
...
def discover_tiles(game_dir: Path, build: str) -> list[TileSource]:
    """Documentation translated to English.
1. cell_X_Y.png ()
2. (b42: biomemap, b41: pyramid.zip)"""
    # cell
...
def discover_map_bounds(game_dir: Path) -> list[dict]:
    """game_dir/media/maps/*/
lotheader worldmap XML

dict
name
cell_bounds: [min_x, max_x, min_y, max_y]
worldmap: "worldmap.xml" ()
worldmap_forest: "worldmap-forest.xml" ()
_worldmap_src: worldmap.xml Path ( manifest)
_worldmap_forest_src: worldmap-forest.xml Path ()"""
    maps_root = game_dir / "media" / "maps"
    if not maps_root.exists():
        return []
...
def convert_thumbs(
    ...
    """output_dir/thumbs/{map_name}.{ext}"""
...
def copy_worldmaps(
    ...
    """worldmap.xml worldmap-forest.xml output_dir

Documentation translated to English.
Documentation translated to English.

Args
maps: discover_map_bounds() map
output_dir: ( resources/map_tiles/b41/vanilla/)
...
(converted_count, skipped_count, total_bytes)"""
    ...
    total = len(tiles)
    report_interval = max(1, total // 20)  # 5%
...
def generate_manifest(
    ...
    """manifest.json"""t_key = f"{build}_vanilla"
